...
func Init(args []string) error {
	...
	simpleDir := filepath.Join(projectDir, ".eacd")
	configPath := filepath.Join(simpleDir, "config.yaml")

	if _, err := os.Stat(configPath); err == nil && !reinit {
		fmt.Println("A .eacd/config.yaml already exists. Run with --reinit to overwrite.")
		return nil
	}

	fmt.Println("Welcome to eacd init. Let's set up your deployment configuration.")
	fmt.Println()
		...
		if err := huh.NewForm(
			huh.NewGroup(
				huh.NewInput().
					...
					Description("Relative to project root. e.g. .eacd/my-tool.service").
					Placeholder(".eacd/my-tool.service").
					Value(&unitFile),
				huh.NewConfirm().
	...
	if hasLocalHook {
		localHookPath = ".eacd/local-pre.sh"
	}

	// --- Generate files ---
	if err := os.MkdirAll(simpleDir, 0755); err != nil {
		return fmt.Errorf("creating .eacd/: %w", err)
	}
	...
	fmt.Println("Created .eacd/config.yaml")
	...
	if preAction != "none" && preAction != "" {
		...
		fmt.Println("Created .eacd/stop.sh")
	}
	...
	if postAction != "none" && postAction != "" {
		...
		fmt.Println("Created .eacd/start.sh")
	}
	...
	if localHookPath != "" {
		...
		fmt.Println("Created .eacd/local-pre.sh")
	}

	// Ensure .eacd/ is excluded from git
	if err := ensureGitignore(projectDir); err != nil {
		fmt.Printf("warning: could not update .gitignore: %v\n", err)
	} else {
		fmt.Println("Updated .gitignore (.eacd/ excluded)")
	}
	...
	step := 1
	if prefillToken == "" {
		fmt.Printf("  %d. Set your auth token: export EACD_TOKEN=<your-token>\n", step)
		step++
	} else {
		fmt.Printf("  %d. Token is in .eacd/config.yaml (move to EACD_TOKEN env var for better security)\n", step)
		step++
	}
	fmt.Printf("  %d. Run: eacd deploy\n", step)
	step++
	fmt.Printf("  %d. Optionally create .eacd/inventory.yaml to manage system packages\n", step)
...
func buildConfigYAML(name, server, token, src, dest string, excludes []string, unitFile string, enableService, restartService bool, preAction, postAction, localHook string, hasSystemd bool) string {
	...
	if token != "" {
		...
		sb.WriteString("# Move to EACD_TOKEN env var for better security.\n")
		sb.WriteString("token: " + token + "\n")
	} else {
		sb.WriteString("# token: your-token  # Use EACD_TOKEN env var instead\n")
	}
	sb.WriteString("\n")
	...
	if preAction != "none" && preAction != "" {
		sb.WriteString("  server_pre: .eacd/stop.sh\n")
	}
	if postAction != "none" && postAction != "" {
		sb.WriteString("  server_post: .eacd/start.sh\n")
	}
	if localHook == "" && (preAction == "none" || preAction == "") && (postAction == "none" || postAction == "") {
		sb.WriteString("  # local_pre: .eacd/local-pre.sh\n")
		sb.WriteString("  # server_pre: .eacd/stop.sh\n")
		sb.WriteString("  # server_post: .eacd/start.sh\n")
	}
...
// ensureGitignore adds ".eacd/" to the project's .gitignore if not already present.
